---
description: Where Big Dog Math stands and the one thing to do next, ranked by classroom risk. Add "sweep" to catalog what this conversation named, or "sync" to reconcile the lists.
argument-hint: "[sweep | sync | <area>]"
---

# /status - where Big Dog Math is, and the one thing to do next
...
**Mode for this run: `$ARGUMENTS`** - empty means the default status pass.
`sync` reconciles the written lists against the code. `sweep` reads THIS
conversation - whichever one the command was invoked in - for changes that were
named out loud and never written down, and catalogs them. Anything else is an
area name (a surface or feature) and scopes the status read to that one thing.
