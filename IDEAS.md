...
Moved out of "Next up" by /status sync 2026-08-05 - each verified against the
code, not the list. ROADMAP.md Live is the full record; kept here as one line so
the queue stops re-proposing them.

- **Order of Operations — GEMS** - SHIPPED. /order-of-operations, src/components/OrderOfOperations.tsx
- **Combining Like Terms** - SHIPPED. /combine-like-terms, src/components/CombineLikeTerms.tsx
- **Number Line → representations + integer hops** - SHIPPED. /number-line-plus, src/components/NumberLineTool.tsx (type Rep = fraction|decimal|percent at :20)
- **Ratios → Proportion builder** - SHIPPED. /proportions, src/components/ProportionBuilder.tsx
- **Percent — parts & wholes** - SHIPPED. /percent-bar, src/components/PercentBar.tsx (type Unknown = part|whole|percent)
