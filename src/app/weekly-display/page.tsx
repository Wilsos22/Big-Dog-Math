...
import { selectedSuccessCriterion } from "@/lib/successCriterion";
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: 26 }}>
      ...
      <div style={{
        ...
        {statement ? (
          ...
          <MissingCopy label="No success criterion chosen for today." theme={theme} />
        )}
      </div>
...
export default function WeeklyDisplayPage() {
  ...
  const board = useMemo(() => {
    ...
    return {
      ...
      hasReveal,
      // ONE criterion a day, from the chosen Notion property. selectedSuccessCriterion
      // (not publicSuccessCriterion) because the latter falls back to the teacher's
      // setup prompt, and "Choose one I can statement in Notion." must never reach a wall.
      criterion: selectedSuccessCriterion(activeLesson?.selectedSuccessCriterion),
    };
  }, [activeLesson]);
